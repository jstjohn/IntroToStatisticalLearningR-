...
Each method chose at least a superset of the correct X polynomials. BIC chose the correct ones (X,X^2,X^3). Cp added in X^9, while Adjusted R2 added in X^9 and X^10 in addition.


### Part d

```{r fig.height=11,fig.width=11}
regfit.full=regsubsets(Y~poly(X,10),data=dat,nvmax=10,method="forward")
reg.summary=summary(regfit.full)
...
The same number of parameters were chosen by each method, however now the spurious parameters changed with Cp and Adjusted R2. Now Cp uses X^5 as the extra, and Adjusted R2 uses X^4 and X^5. 
...
library(glmnet)
dat.mat=model.matrix(Y~poly(X,10),data=dat)[,-1]
...
Using the optimal lambda chosen by CV, lasso regression choses up to a 3nd degree polynomial (X,X^2,X^3), which is the correct model on the underlying data.
...
This model was difficult for the methods to deal with. They both chose to include X^1...7 in the best model, despite the fact that X^7 was the only one used.
